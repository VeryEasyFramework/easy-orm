...
function messageParser(data: Uint8Array) {
    ...
    while (offset < data.byteLength) {
        ...
        const messageLength = new DataView(data.buffer).getUint32(
            offset + 1,
            false,
        );
        ...
        if (firstByte === "D") {
            const columnCount = new DataView(message.buffer).getInt16(0, false);
            const columnLength = new DataView(message.buffer).getInt32(
                2,
                false,
            );
            console.log({ columnCount });
            ...
            switch (messageType) {
                ...
                case "E": {
                    ...
                    while (reader.offset < reader.messageLength) {
                        const field = reader.readChar() as
